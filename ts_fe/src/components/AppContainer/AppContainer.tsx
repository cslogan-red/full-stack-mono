...
const AppContainer = () => {
  ...
  const workloadHandler = async (size: WorkerSizeType = WORKER_SIZE.small) => {
    ...
    const result = isSmallMT
      ? await smallWorker({ batchSize: BATCH_SIZE.small, isSmall: true })
      : isLargeMT
        ? await largeWorker({ batchSize: BATCH_SIZE.large })
        : isSmall
          ? await getWorkerDataSet({ batchSize: BATCH_SIZE.small, isSmall: true })
          : await getWorkerDataSet({ batchSize: BATCH_SIZE.large });
    if (result.nextToken.lat > 0) {
      ...
      while (paginatedResult.nextToken.lat > 0) {
        ...
        paginatedResult = isSmallMT
          ? await smallWorker({ startLat, startLng, batchSize: BATCH_SIZE.small, isSmall: true })
          : isLargeMT
            ? await largeWorker({ startLat, startLng, batchSize: BATCH_SIZE.large })
            : isSmall
              ? await getWorkerDataSet({ startLat, startLng, batchSize: BATCH_SIZE.small, isSmall: true })
              : await getWorkerDataSet({ startLat, startLng, batchSize: BATCH_SIZE.large });
  ...
  const multiThreadedCheckedHandler = () => {
